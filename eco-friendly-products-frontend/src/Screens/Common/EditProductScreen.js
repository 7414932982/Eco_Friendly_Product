...
const EditProductScreen = (props) => {
    ...
    const url_image = "http://192.168.0.103/src/Images"
    const header = {
        headers: {
    ...
    const [imageFile,setImageFile] = useState((productDetail.product.imageName && url_image + `/${productDetail.product.imageName}` )|| AddProduct)
    const [image,setImage] = useState(null)
    
    const units = ["gm","kg","pc","pkt","dozen"]
